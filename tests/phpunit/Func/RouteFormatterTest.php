...
class RouteFormatterTest extends WP_UnitTestCase {
    ...
    public function test_auth_level_authenticated_for_array_callback(): void {
        $cb    = [ new \stdClass(), 'some_method' ];
        $route = $this->format_with_permission_callback( $cb );
        $this->assertSame( 'authenticated', $route['endpoints'][0]['auth_level'] );
    }

    public function test_auth_level_custom_for_unknown_string(): void {
        $route = $this->format_with_permission_callback( 'my_custom_permission_check' );
        $this->assertSame( 'custom', $route['endpoints'][0]['auth_level'] );
    }

    public function test_auth_level_none_when_callback_missing(): void {
        $route = $this->format_endpoint( [] );
    ...
    public function test_permission_label_public(): void {
        $route = $this->format_with_permission_callback( '__return_true' );
        $this->assertSame( 'Public', $route['endpoints'][0]['permission_label'] );
    }

    public function test_permission_label_blocked(): void {
        $route = $this->format_with_permission_callback( '__return_false' );
        $this->assertSame( 'Blocked', $route['endpoints'][0]['permission_label'] );
    }

    public function test_permission_label_closure(): void {
        $route = $this->format_with_permission_callback( static fn () => true );
    ...
    public function test_namespace_detected_from_path(): void {
    ...
    public function test_namespace_falls_back_to_core_when_unmatched(): void {
        $route = RouteFormatter::format_route(
            '/',
            [ $this->make_endpoint( '__return_true' ) ],
            [ 'wp/v2' ]
    ...
    public function test_args_are_normalised(): void {
        $endpoint = $this->make_endpoint( '__return_true', [
            'page' => [
                'type'        => 'integer',
                'default'     => 1,
        ...
        $route = RouteFormatter::format_route( '/wp/v2/posts', [ $endpoint ], [ 'wp/v2' ] );
    ...
    private function format_with_permission_callback( $callback ): array {
    ...
    private function format_endpoint( array $extra ): array {
        return RouteFormatter::format_route(
            '/wp/v2/posts',
            [ array_merge( [ 'methods' => [ 'GET' => true ] ], $extra ) ],
            [ 'wp/v2' ]
        );
    }
